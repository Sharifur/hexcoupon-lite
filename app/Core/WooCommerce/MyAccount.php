...
class MyAccount
{
	...
	public function coupon_menu_in_my_account_page( $all_menu_links )
	{
		$all_menu_links = array_slice( $all_menu_links, 0, 5, true )
			+ [ 'all-coupons' => esc_html__( 'All Coupons', 'hexcoupon-advance-coupons-for-woocommerce' ) ]
	...
	public function all_coupon_list()
	{
		...
		foreach ( $coupon_posts as $coupon_post ) {
			?>
				<P>
				<?php printf( esc_html__( '%s', 'hexcoupon-advance-coupons-for-woocommerce' ),  esc_html( $coupon_post->post_title ) ); ?>
				</P>
			<?php
		}
	}
	...
	public function coupon_page_endpoint_content()
	{
		?>
			<header class="woocommerce-Address-title title">
				<h3><?php echo esc_html__( 'All Available Coupons', 'hexcoupon-advance-coupons-for-woocommerce' ); ?></h3>
			</header>
		<?php
